...
//TODO Shivansh, In the parameter tables following this section, for the main template under QSS3KeyPrefix, we say AllowedPattern: ^([0-9a-zA-Z-.]+/)*$. Are these parentheses above supposed to be here? We don't include parens for this parameter in IMC-workload.template.yaml or in this Quip: https://quip-amazon.com/lIOaA1eoAGDz/SigV4-Quick-Start-Changes.

//TODO Shivansh, In the parameter tables, for both templates under EdgeDeviceID, we give the label as "Name for the edge device." The deployment guide refers to "physical edge devices" plural. Which device are we naming here ... maybe the industrial PC? And do we change this name only for physical deployments?
