...
//TODO Shivansh, In the parameter tables following this section, in IMC-main.template.yaml under QSS3KeyPrefix, we say AllowedPattern: ^([0-9a-zA-Z-.]+/)*$. Are these parentheses above supposed to be here? We don't include parentheses for this parameter in IMC-workload.template.yaml or in this Quip: https://quip-amazon.com/lIOaA1eoAGDz/SigV4-Quick-Start-Changes. - Fixed.
